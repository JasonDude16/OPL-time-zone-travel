...
```{r}
mod <- lmer(
  TotalKGPercChange ~ StateMeetTzDiff + TotalKg + MeetNumber + 
    AgeClass + BodyweightKg + cDays + (1 | Name), 
  data = df_clean
)

mod_bench <- lmer(
  BenchPercChange ~ StateMeetTzDiff + Best3BenchKg + MeetNumber + 
    AgeClass + BodyweightKg + cDays + (1 | Name), 
  data = df_clean
)

mod_squat <- lmer(
  SquatPercChange ~ StateMeetTzDiff + Best3SquatKg + MeetNumber + 
    AgeClass + BodyweightKg + cDays + (1 | Name), 
  data = df_clean
)

mod_deadlift <- lmer(
  DeadliftPercChange ~ StateMeetTzDiff + Best3DeadliftKg + MeetNumber + 
    AgeClass + BodyweightKg + cDays + (1 | Name), 
  data = df_clean
)
```
