...
13,130 repeat U.S. lifters (46% women) met inclusion criteria. Performance gains were greatest when competing locally (\~11–12 kg) and were attenuated when crossing time zones, with the largest reduction observed after crossing 1–2 time zones, independent of travel direction (Eastward, 1 time zone: β = −4.0 kg, 95% CI: −6.0 to −2.0, p\<0.01; Westward, 1 time zone: β = −4.0 kg, 95% CI: −5.5 to −2.4, p\<0.01).
...
df_filt_nosplit <- df_filt |> 
  filter(
    !(State %in% split_states),
    !(MeetState %in% split_states)
  )

# use UTC offsest difference to derive direction of travel
df_tz <- df_filt_nosplit |>
  mutate(
...
mod <- lmer(
  DeltaTotal ~ StateMeetTzDiff + MeetNumber + AgeClass + Sex + BodyweightKg + cDays + (1 | Name), 
  data = df_clean
)
```
